...
paths = {
    ...
    "sources_local": localfs.filepath("images", "pix2pix-sources"),
    "dataset_local": localfs.filepath("images", "pix2pix-dataset"),
    "dataset_remote": "uploads/pix2pix-dataset"
}

if not localfs.exists(paths["images_local"]): 
    localfs.mkdir(paths["images_local"])
    
if not localfs.exists(paths["dataset_local"]): 
    localfs.mkdir(paths["dataset_local"])
...
def imagepairs(args):
    ...
    if len(args) < 4:
        raise RuntimeError("4 arguments required")
    source_dir = paths["sources_local"]
    source_a = localfs.find_first(source_dir, ["A.jpg", "A.jpeg", "A.png"])
    source_b = localfs.find_first(source_dir, ["B.jpg", "B.jpeg", "B.png"])
    ...
    num_x = int(args[2])
    num_y = int(args[3])
    output_dir_a = localfs.filepath(paths["dataset_local"], "A")
    output_dir_b = localfs.filepath(paths["dataset_local"], "B")
    log("Extracting subimages...")
    localfs.rm(paths["dataset_local"])
    localfs.mkdir(paths["dataset_local"])
    ...
    pairs_dir_path = localfs.filepath(paths["dataset_local"], "pairs")
    ...
    for fname in localfs.ls(pairs_dir_path):
        ...
        fpath_new = localfs.filepath(paths["dataset_local"], fname)
        localfs.mv(fpath, fpath_new)
    localfs.rm(pairs_dir_path)
    log("Done. Image pairs are in the directory '{}'".format(paths["dataset_local"]))
...
def upload():
    ...
    if localfs.isEmpty(paths["dataset_local"]):
        raise RuntimeError("Missing images in {}".format(paths["dataset_local"]))
    for filename in localfs.ls(paths["dataset_local"]):
        if " " in filename:
    ...
    imageutils.strip_img_alpha_channel(paths["dataset_local"])
    log("Uploading. Please wait...")
    spell.upload(paths["dataset_local"], paths["dataset_remote"])
    log("Upload complete.")
...
def train(pix2pix_args, machine_type="K80"):
    ...
    scale_size = 512
    if pix2pix_args:
        if not utils.isInteger(pix2pix_args[0]):
            raise ValueError("max_epochs parameter must be an integer")
        max_epochs = int(pix2pix_args[0])
        if len(pix2pix_args) >= 2:
            if not utils.isInteger(pix2pix_args[1]):
                raise ValueError("display_freq parameter must be an integer")
            display_freq = int(pix2pix_args[1])
    resume = len(pix2pix_args) > 1 and pix2pix_args[1] == "resume"
    if resume:
        if len(pix2pix_args) < 3:
            run_id = localfs.cacheGet("pix2pixTrainingRunId")
            if not run_id:
                raise RuntimeError("resume training requires a run number")
            if not utils.isInteger(run_id):
                raise ValueError("Run id must be an integer")
        else:
            run_id = pix2pix_args[2]
        checkpoint_dir = "runs/{}/output".format(run_id)
    spell.validateMachineType(machine_type)
...
def downloadlatest(args):
    ...
    local_dir = localfs.filepath(paths["images_local"], "pix2pix-results")
...
def export(args, machine_type="CPU"):
    ...
    if args:
        ...
        run_id = localfs.cacheGet("pix2pixTrainingRunId")
    if not run_id:
        raise RuntimeError("export command requires a run id")
    spell.validateMachineType(machine_type)
    ...
    log("Download complete. The model.pict file in the models directory \
            {} is ready for use in tensorflowjs.".format(localfs.filepath("models", "pix2pix")))
